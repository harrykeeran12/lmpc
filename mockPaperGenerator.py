...
class MockPaperGenerator:
    ...
    def __init__(
        ...
        if MODELNAME not in INSTALLEDMODELS:
            ...
            # ollama.pull(MODELNAME, stream=False)
            pass
        try:
    ...
    def rewrite(self):
        ...
        NOTESREWRITE = f"""Instruct: Rewrite, reorganise and summmarise these notes below into one simple clear paragraph.
        Make sentences simple. Do not add any other information. Do not write the questions.
        Write each note in a definition-example-explanation structure. Make sure that the data is all in plain-text. \n \n {
            self.NOTES
        }"""
        print(NOTESREWRITE)

        rewriteNotes = ollama.generate(model=self.MODELNAME, prompt=NOTESREWRITE)
    ...
    def prompt(self):
        ...
        self.SYSTEMPROMPT = f"""Instruct: Generate {self.QUESTIONNUMBER} questions, in a list.
        Each question should be written as a string, and should be separated by one new line character each.
    ...
    def generate(self):
        ...
        generation = ollama.generate(model=self.MODELNAME, prompt=self.SYSTEMPROMPT)
        # print(generation["response"])
        self.generated_questions = [
            question.replace('"', "") for question in generation["response"].split("\n")
        ]
            ...
            with open("questions.tex", "w") as questionTemplate:
                if self.QUESTIONTYPE == "short":
                    ...
                    for question in self.generated_questions:
                        ...
                        #   my_file.write(f"\\newline")
                        questionTemplate.write("\\vspace{{0.5in}}")
    ...
    def compileTeX(self, texFile: str):
        ...
        if texFile[-3:] == "tex":
            ...
            # Double compilation to fix the table.
            subprocess.run(
                ["pdflatex", "-interaction", "nonstopmode", texFile], shell=False
            )
            # Check path to make sure it ran properly.
            if not Path("exam_1.pdf").exists():
